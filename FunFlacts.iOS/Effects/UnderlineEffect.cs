    ...
    public class UnderlineEffect : PlatformEffect
    {
        ...
        private void SetUnderline(bool on)
        {
            ...
            if (label != null)
            {
                ...
                var rangeIt = new NSRange(0, 0);
                if (text != null)
                {
                     rangeIt = new NSRange(0, text.Length);
                }

                if (on)
                {
                    text?.AddAttribute(UIStringAttributeKey.UnderlineStyle, NSNumber.FromInt32((int)NSUnderlineStyle.Single), rangeIt);
                }
                else
                {
                    text?.RemoveAttribute(UIStringAttributeKey.UnderlineStyle, rangeIt);
                }
            }
        }
    }
